...
#' produced by an error while parsing values. Valid values are 'null' which
#' replaces them with null; and 'fail' which will result in an exception.
#' @param out_of_range_datetime How to handle date-time values that are outside
#' the range supported by Pandas. Valid values are 'null' which replaces them
#' with null; and 'fail' which will result in an exception.
#' @return A dataframe.
#' @export
#' @md
load_dataset_into_data_frame <- function(dataset, on_error = NULL,
                                         out_of_range_datetime = NULL)	{
  dataset$to_pandas_data_frame(on_error, out_of_range_datetime)
}
